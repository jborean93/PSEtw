...
{
    private FieldInfo[]? _keywords;
    private FieldInfo[]? _levels;

    public Guid Id { get; }

    public string Name { get; }

    public FieldInfo[] Keywords
    {
        get
        {
            if (_keywords == null)
            {
                _keywords = GetProviderFieldInfo(Id, EventFieldType.EventKeywordInformation);
            }
            return _keywords;
        }
    }

    public FieldInfo[] Levels
    {
        get
        {
            if (_levels == null)
            {
                _levels = GetProviderFieldInfo(Id, EventFieldType.EventLevelInformation);
            }
            return _levels;
        }
    }

    public EventProvider(Guid id, string name)
    {
        Id = id;
        Name = name;
    }

    internal static unsafe FieldInfo[] GetProviderFieldInfo(Guid provider, EventFieldType fieldType)
    {
...
internal static class PSETWGlobals
{
    ...
    private static EventProvider[]? _installedProviders = null;
    private static string[]? _traceSessionNames = null;
    ...
    public static EventProvider[] InstalledProviders
    {
        get
        {
            if (_installedProviders == null)
            {
                unsafe
                {
                    _installedProviders = GetInstalledProviders();
                }
            }
    ...
    public static string[] TraceSessionNames
    {
        get
        {
            if (_traceSessionNames == null)
            {
                unsafe
                {
                    _traceSessionNames = GetTraceSessionNames();
                }
            }

            return _traceSessionNames;
        }
    }

    private unsafe static EventProvider[] GetInstalledProviders()
    {
        List<EventProvider> finalRes = new();
        ...
        try
        {
            while (true)
            {
                ...
                foreach (Tdh.TRACE_PROVIDER_INFO info in providers)
                {
                    ...
                    finalRes.Add(new(info.ProviderGuid, providerName));
                }
    ...
    private unsafe static string[] GetTraceSessionNames()
    {
        int traceCount = 64;
